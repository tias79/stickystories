module US exposing (Id, Model, NewTaskState(..), T, USStage(..), count, filter, fromJson, init, move, new, toDeltaJson, updateDescription, updateName, updateNewTaskState, updateUSStage)
...
new model =
    let
        ...
        newUS =
            ...
            , order =
                case List.sortBy .order (Dict.values model.userStories) |> List.reverse |> List.head of
                    Just us ->
                        us.order + 10.0 |> floor |> toFloat
...
update model usId modifier =
    let
        uuid =
            EntityUUID.toString usId

        maybeUs =
            Dict.get uuid model.userStories
                |> Maybe.map modifier
    in
    case maybeUs of
        Just us ->
            ( [ us ], { model | userStories = Dict.insert uuid us model.userStories } )
...
updateName model usId name =
    update model (Debug.log "updateName usId" usId) (\us -> { us | name = name })


updateDescription : Model -> Id -> String -> ( List T, Model )
updateDescription model usId description =
    update model (Debug.log "updateDescription usId" usId) (\us -> { us | description = description })


updateUSStage : Model -> Id -> USStage -> ( List T, Model )
updateUSStage model usId stage =
    update model (Debug.log "updateUSStage usId" usId) (\us -> { us | stage = stage })


updateNewTaskState : Model -> Id -> NewTaskState -> Model
updateNewTaskState model usId state =
    Tuple.second <| update model (Debug.log "newTaskState usId" usId) (\us -> { us | newTaskState = state })
...
move model srcId targetId =
    let
        src =
            Dict.get (EntityUUID.toString srcId) model.userStories

        target =
            Dict.get (EntityUUID.toString targetId) model.userStories
    in
                ...
                case target of
                    ...
                    Just targetUs ->
                        let
                            ...
                            newOrder =
                                targetOrder + (adjacentOrder - targetOrder) / 2

                            updatedSrcUS =
                                { srcUs | order = newOrder }
                        in
...
toDeltaJson model us =
    let
        maybeOrigUs =
            Dict.get (EntityUUID.toString us.id) model.userStories
    ...
    case maybeOrigUs of
        ...
        Just origUs ->
            Encode.object
                ([ id ]
                    ++ (if us.name /= origUs.name then
                            [ name ]

                        else
                            []
                                ++ (if us.description /= origUs.description then
                                        [ description ]

                                    else
                                        []
                                            ++ (if Debug.log "us.stage" us.stage /= Debug.log "origUs.stage" origUs.stage then
                                                    [ stage ]

                                                else
                                                    []
                                                        ++ (if us.order /= origUs.order then
                                                                [ order ]
...
fromJson model id json =
    let
        templateUS =
            case Dict.get (EntityUUID.toString id) model.userStories of
                Just us ->
                    us

                Nothing ->
                    Tuple.first (new model)

        maybeNewUS =
            Decode.decodeValue (usDecoder templateUS) json
    in
    case maybeNewUS of
        Err error ->
